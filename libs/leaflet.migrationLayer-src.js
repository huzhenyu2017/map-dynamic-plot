(function (window) {
    ...
    L.MigrationLayer = L.Polyline.extend({
        options: {
            map: {},
            data: {},
            pulseRadius: 25,
        ...
        initialize: function (options,latlngs) {
...
//          });
		...
		_containsPoint: function (p) {
			var point=this._map.layerPointToContainerPoint(p);
			var inside = false;
			var arc=this.migration.store.arcs[0];
			
			var minBounds=this._map.layerPointToContainerPoint(this._pxBounds.min);
			var maxBounds=this._map.layerPointToContainerPoint(this._pxBounds.max);
			var containerBounds=L.bounds(minBounds,maxBounds);
			if (!containerBounds.contains(point)) { return false; }
	
			// 与圆心的距离
			var distance=Math.sqrt(Math.pow((point.x-arc.centerX),2)+Math.pow((point.y-arc.centerY),2))-arc.radius;
			if(Math.abs(distance)<=this.options.pulseRadius){
				inside=true;
			};
		...
		setLatLngs: function(latlngs){
			this._setLatLngs(latlngs);
			var data=this._getData(latlngs);
			this.setData(data);
		},
			//整理flowline data格式
		...
		setStyle: function (style) {
			...
			if(style.color!=undefined){
				this._data[0].color=style.color;
			};
			if(style.arcWidth!=undefined){
				this._style.arc.width=style.arcWidth;
			};
			if(style.pulseRadius!=undefined){
				this._style.pulse.radius=style.pulseRadius;
			};
			if(style.pulseBorderWidth!=undefined){
				this._style.pulse.borderWidth=style.pulseBorderWidth;
			};			
			this._draw();
    ...
    L.migrationLayer = function (options,latlngs) {
        return new L.MigrationLayer(options,latlngs)
    }
})(window)